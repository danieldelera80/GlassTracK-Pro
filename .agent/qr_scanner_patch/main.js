...
function onRender(event) {
    if (!window.rendered) {
        ...
        docReady(function () {
            // Dar tiempo al iframe para que el video tenga ancho real
            setTimeout(function () {
                var w = video.parentElement ? video.parentElement.clientWidth : 0;
                if (w < 50) w = video.clientWidth;
                if (w < 50) w = 300;

                var scanH = Math.round(w * 0.55);
                video.setAttribute('height', scanH);
                Streamlit.setFrameHeight(scanH + 50);

                // qrbox ancho y bajo: optimo para codigos 1D (Code 128, Code 39)
                var boxW = Math.round(w * 0.82);
                var boxH = Math.round(boxW * 0.28);
                ...
                const config = { fps: 10, qrbox: { width: boxW, height: boxH } };
