            ...
            if pass_input == ADMIN_PASSWORD:
                if "[URGENTE]" not in orden_actual:
                    ...
                    with conn.session as s:
                        ...
                        s.commit()
                    st.success("🚀 Prioridad inyectada exitosamente.")
                    st.rerun()
                else:
        ...
        if st.button("🗑️ Eliminar", type="primary", use_container_width=True):
            if pass_input == ADMIN_PASSWORD:
                with conn.session as s:
                    ...
                    s.commit()
                st.success("✅ Orden borrada permanentemente.")
                st.rerun()
            elif pass_input:
                st.error("Contraseña incorrecta.")
...
with c1:
    ...
    <div class="glass-metric">
        <div class="glass-title">📦 Órdenes Hoy</div>
...
# ── TAB 1: PRODUCCIÓN ────────────────────────────────────────────────────────
with tab_prod:
    # ── Alerta de órdenes urgentes activas (banner + botón abrir) ────────────
    ...
    if df_total.empty:
        ...
        if busqueda.strip():
            ...
            if df_vista.empty:
                st.warning(f"⚠️ No se encontraron registros para **{busqueda}**")
            else:
        ...
        if not busqueda.strip():
            ...
            leg2.markdown(
                ...
                'border-radius:6px;font-size:12px;text-align:center;">⏳ Terminado — esperando entrega</div>',
                unsafe_allow_html=True,
            )
            leg3.markdown(
                '<div style="background:#1a4a2e;color:#4ada75;padding:6px 12px;'
                'border-radius:6px;font-size:12px;text-align:center;">✅ Orden entregada al cliente</div>',
                unsafe_allow_html=True,
            )
            st.write("")
...
with tab_rend:
    if df_prod_hoy.empty:
        st.info("📭 Sin movimientos registrados hoy para mostrar el rendimiento.")
    else:
    ...
    with tab_admin:
        ...
        if orden_seleccionada_str:
            ...
            <div style="background: #1e293b; border: 1px solid #334155; padding: 20px; border-radius: 12px; margin-bottom: 25px; box-shadow: 0 4px 15px rgba(0,0,0,0.2);">
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 12px;">
                    <div style="flex: 1; min-width: 120px; background: #0f172a; padding: 12px; border-radius: 8px; border-left: 3px solid #6366f1;">
                        <p style="color: #94a3b8; font-size: 11px; margin: 0 0 4px 0; text-transform: uppercase; font-weight: 600;">Operario</p>
                    ...
                    <div style="flex: 1; min-width: 120px; background: #0f172a; padding: 12px; border-radius: 8px; border-left: 3px solid #14b8a6;">
                        <p style="color: #94a3b8; font-size: 11px; margin: 0 0 4px 0; text-transform: uppercase; font-weight: 600;">Sector Actual</p>
                    ...
                    <div style="flex: 1; min-width: 120px; background: {bg_tiempo}; padding: 12px; border-radius: 8px; {borde_tiempo}">
                        <p style="color: #94a3b8; font-size: 11px; margin: 0 0 4px 0; text-transform: uppercase; font-weight: 600;">Tiempo en Sector</p>
                    ...
                    <div style="flex: 1; min-width: 120px; background: #0f172a; padding: 12px; border-radius: 8px; border-left: 3px solid #f59e0b;">
                        <p style="color: #94a3b8; font-size: 11px; margin: 0 0 4px 0; text-transform: uppercase; font-weight: 600;">Carro / Lado</p>
                    ...
                    <div style="flex: 1; min-width: 120px; background: #0f172a; padding: 12px; border-radius: 8px; border-left: 3px solid #8b5cf6;">
                        <p style="color: #94a3b8; font-size: 11px; margin: 0 0 4px 0; text-transform: uppercase; font-weight: 600;">Fecha / Hora</p>
            ...
            st.markdown("""
                <style>
                    div[data-testid="stButton"] button:has(div p:contains("💾 GUARDAR CAMBIOS")) {
                        ...
                        padding: 1rem !important; font-size: 24px !important;
            ...
            if guardar:
                try:
                    ...
                    with conn.session as s:
                        ...
                        s.commit()
                    st.success(f"✅ ¡Cambios guardados! **{orden_editar}** actualizada.", icon="🚀")
                    st.cache_data.clear()
                    st.rerun()
            ...
            with st.expander(f"🗑️ Eliminar todos los registros de la orden **{orden_editar}**"):
                st.warning(f"⚠️ Borrará **permanentemente** todas las filas de `{orden_editar}`. No se puede deshacer.")
