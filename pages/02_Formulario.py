...
def procesar_orden(valor):
    ...
    # ═══════════════════════════════════════════════════════════════
    # ORDEN NUEVA → Validar si es una pieza inexistente de orden existente
    # ═══════════════════════════════════════════════════════════════
    match_pieza = re.match(r'^(.+)-(\d+)$', orden_normalizada)
    if match_pieza:
        orden_base_real = match_pieza.group(1)
        df_base = conn.query(
            """
            WITH ultimo_estado AS (
                SELECT DISTINCT ON (TRIM(orden)) 
            ...
            """,
            params={
                "exacto": orden_base_real,
                "patron_pieza": f"^{re.escape(orden_base_real)}-[0-9]+$",
                "urg_exacto": f"[URGENTE] {orden_base_real}",
                "urg_pieza": f"^\\[URGENTE\\] {re.escape(orden_base_real)}-[0-9]+$",
                "inc_exacto": f"[INCIDENCIA] {orden_base_real}",
                "inc_pieza": f"^\\[INCIDENCIA\\] {re.escape(orden_base_real)}-[0-9]+$"
            },
            ttl=0
        )
        if not df_base.empty:
            nums_existentes = []
            for ord_str in df_base["orden"].unique():
                ord_clean = _PFX_RE.sub("", str(ord_str)).strip()
                m_num = re.match(rf'^{re.escape(orden_base_real)}-(\d+)$', ord_clean)
                if m_num:
                    nums_existentes.append(m_num.group(1))
                elif ord_clean == orden_base_real:
                    nums_existentes.append("Base")
            
            st.session_state.pieza_inexistente = {
                "escaneada": orden_buscada,
                "orden_base": orden_base_real,
                "piezas_que_si_existen": nums_existentes
            }
            return
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # ════════════════════════════════════════════════════════════════════
    # PIEZA INEXISTENTE (Validación previa a Orden Nueva)
    # ════════════════════════════════════════════════════════════════════
    if st.session_state.get("pieza_inexistente"):
        _pi = st.session_state.pieza_inexistente
        st.error(f"❌ La pieza **{_pi['escaneada']}** no existe. La orden **{_pi['orden_base']}** tiene estas piezas: {', '.join(_pi['piezas_que_si_existen'])}")
        if st.button("OK", key="btn_ok_inexistente"):
            st.session_state.pieza_inexistente = None
            st.rerun()
        st.stop()
    ...
    if st.session_state.get("orden_existe_trigger"):
        ...
        <div style="background:#1e3a8a;border:2px solid #3b82f6;border-radius:12px;
            ...
            <div style="font-size:14px;color:#bfdbfe;text-align:center;line-height:1.6;">
                Orden <b>{_oe['orden_base']}</b> — <b>{_oe['total_piezas']} pieza(s) existente(s)</b>
            </div>
        </div>
        """, unsafe_allow_html=True)
        
        st.write("Seleccioná las piezas que querés traer a tu sector:")
        
        piezas_seleccionadas = []
        for p in _oe["piezas"]:
            en_mi_sector = p['sector'] in [f"En Proceso en {st.session_state.sector_confirmado}", f"Enviado a {st.session_state.sector_confirmado}"]
            col_chk, col_info = st.columns([1, 4])
            with col_chk:
                if en_mi_sector:
                    st.checkbox("", value=False, disabled=True, key=f"chk_dis_{p['orden']}")
                else:
                    if st.checkbox("", value=False, key=f"chk_existe_{p['orden']}"):
                        piezas_seleccionadas.append(p)
            with col_info:
                st.markdown(f"**{p['orden']}**")
                if en_mi_sector:
                    st.caption("Ya está en tu sector")
                else:
        ...
        with col_btn2:
            if st.button(f"↘️ TRAER SELECCIONADAS A {st.session_state.sector_confirmado}", use_container_width=True, type="primary", key="btn_traer_multi"):
                if not piezas_seleccionadas:
                    st.warning("No seleccionaste ninguna pieza.")
                else:
                    creadas = 0
                    errores = []
                    for p in piezas_seleccionadas:
                        ok, err = guardar_registro(
                            p['orden'], p['carro'], p['lado'] or "A",
                            st.session_state.op_confirmado,
                            f"En Proceso en {st.session_state.sector_confirmado}"
                        )
                        if ok:
                            creadas += 1
                            agregar_historial(p['orden'], f"En Proceso en {st.session_state.sector_confirmado}")
                        else:
                            errores.append(f"{p['orden']}: {err}")
                    
                    st.session_state.orden_existe_trigger = None
                    if creadas > 0:
                        st.session_state.ultimo = {
                            "orden": piezas_seleccionadas[-1]['orden'],
                    ...
                    st.session_state.ord_n += creadas
                    if errores:
                        st.warning(f"✅ {creadas} tomadas. ⚠️ {len(errores)} errores")
                        import time
                        time.sleep(2)
                    st.rerun()
